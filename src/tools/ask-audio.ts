...
export async function handleAskAudio(args: {
  file_path: string;
  question: string;
  ...
  const mimeType = resolveAudioMimeType(ext);
  ...
  if (!mimeType) {
    return {
      content: [
        {
          type: "text",
          text: `Unsupported audio format "${ext}". Supported: ${SUPPORTED_AUDIO_EXTENSIONS.join(", ")}`,
        },
      ],
      isError: true,
    };
